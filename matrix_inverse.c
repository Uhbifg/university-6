...
int
matrix_inverse(double *array, int n, double *inverse, int *vec, int shift, int rank, int total_size, double *row_buffer,
    ...
    for (int i = 0; i < total_size; i++) {
        if (rank == i) {
            for (int col = 0; col < shift; col++) {
                ...
                for (int row = 0; row < n; row++) {
                    if (fabs(array[col + shift * row] - 1) < eps) {
                        vec[col + shift * rank] = row;
                        break;
                    }
                }
            }
        ...
        //MPI_Barrier(MPI_COMM_WORLD);
        MPI_Bcast(vec, n, MPI_INT, i, MPI_COMM_WORLD);
    }
    return 0;
}
